...
use ip_location::{GeoInfo, IpLocator};
...
impl StatsService {
                    ...
                    Ok(msg) => {
                        match msg {
                            ...
                            StatsMessage::InsertArticleStats(base_article_stat) => {
                                ...
                                let (client_ip, geo_info, pseudo_ip): (
                                    String,
                                    Option<GeoInfo>,
                                    String,
                                    ...
                                    Some(ip) => {
                                        ...
                                        let geo_info = match ip_locator.geo_info(ip) {
                                            Ok(info) => Some(info),
                                            Err(e) => {
                                                error!(
                                                    "Error from StatsService for IP Location \
                    - {}",
                                                    e
                                                );
                                                None
                                ...
                                let geo_info: GeoInfo = geo_info.unwrap_or(GeoInfo {
                                    country: String::new(),
                                    region: String::new(),
                                    city: String::new(),
                                });
                                ...
                                let article_stat = ArticleStat {
                                    ...
                                    pseudo_ua: pseudonymize(
                                        &mut pseudonymizer,
                                        &base_article_stat.client_ua,
                                    ),
                                    pseudo_ip,
                                ...
                                if let Err(e) = insert_article_stat(&connection, &article_stat) {
                                    error!(
                                        "Error from StatsService: \
                  could not insert ArticleStats - {}",
                                        e
                                    );
                                }
                            }
    ...
    pub fn insert_article_stats(&self, article_stats: BaseArticleStat) -> Result<()> {
