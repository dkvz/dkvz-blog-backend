...
// won't get replaced.
pub fn relative_links_to_absolute<'a>(source: &'a str, base_url: &str) -> Cow<'a, str> {
    ...
    // surprised too.
    lazy_static! {
        static ref REL_LINK_REGEX: Regex = Regex::new("(?i)(src=\"|href=\")/(.*?)\"").unwrap();
    }

    // That wouldve been too easy:
    //REL_LINK_REGEX.replace_all(source, &format!("$1{}/$2\"", base_url))
    REL_LINK_REGEX.replace_all(source, |caps: &Captures| {
...
pub fn single_link_to_absolute(
    ...
    if !source.as_ref().contains("://") {
        Some(format!("{}{}", base_url.as_ref(), source.as_ref()))
    } else {
        None
    }
}
