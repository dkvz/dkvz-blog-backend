...
use crate::db::{SNIPPET_END, SNIPPET_START};
use crate::utils::text_utils::escape_html;
...
impl From<ImportedArticleDto> for Article {
    fn from(dto: ImportedArticleDto) -> Self {
        ...
        let article_url = serde_utils::empty_string_to_none(if dto.article_url.is_some() {
            dto.article_url
        } else {
            dto.article_url_bis
        });
        Self {
            ...
            thumb_image: dto.thumb_image.and_then(serde_utils::empty_string_to_none),
            ...
            tags: dto
                .tags
                .map(|v| v.into_iter().map(|a| Tag::from(a)).collect())
                .unwrap_or(Vec::new()),
...
impl From<ImportedArticleDto> for ArticleUpdate {
    fn from(dto: ImportedArticleDto) -> Self {
        Self {
            ...
            summary: dto.summary,
            content: dto.content,
            published: dto.published.map(|p| match p {
                true => 1,
                false => 0,
            }),
            tags: dto
...
impl From<Article> for SearchResult {
    fn from(article: Article) -> Self {
        ...
        // Replace < and > by their entity
        // Then replace the snippets markers with a <b> tag
        let summary = escape_html(article.summary)
            .replace(SNIPPET_START, "<b>")
            .replace(SNIPPET_END, "</b>");
...
impl JsonStatus {
    ...
    pub fn new_with_id(status: JsonStatusType, message: &str, id: i32) -> Self {
        Self {
            status: status.to_string(),
            message: String::from(message),
            id: Some(id),
...
impl<'a> RssFeed<'a> {
    ...
    pub fn add_item(&mut self, article: Article) {
        ...
        let link = match article.short {
            1 => helpers::generate_article_url(self.root, self.shorts_root, article.id.to_string()),
        ...
        let media = article.thumb_image.map(|url| {
        ...
        if description.len() > self.max_rss_length {
            description = description.chars().take(self.max_rss_length).collect();
        ...
        let description = text_utils::relative_links_to_absolute(&description, self.root);

        self.items.push(RssFeedEntry {
            title: article.title,
            link,
...
impl<'a> RenderedArticle<'a> {
    pub fn new(article: Article, site: &'a SiteInfo) -> Self {
        let full_article_url = match &article.short {
            1 => {
                helpers::generate_article_url(&site.root, &site.shorts_root, article.id.to_string())
            }
            _ => helpers::generate_article_url(
                &site.root,
                &site.articles_root,
        ...
        // the DTO:
        let compact_publication_date =
            time_utils::timestamp_to_date_string(article.date, time_utils::DateFormat::USCompact);
        ...
        if let Some(thumb_image) = &article_dto.thumb_image {
            if let Some(absolute_url) = text_utils::single_link_to_absolute(thumb_image, &site.root)
            {
                article_dto.thumb_image = Some(absolute_url);
            }
        }
        article_dto.summary =
            text_utils::relative_links_to_absolute(&article_dto.summary, &site.root).to_string();
        article_dto.content = article_dto
            .content
            .map(|c| text_utils::relative_links_to_absolute(&c, &site.root).to_string());
