...
pub fn map_article(
    ...
    "articles.content"
    */
    let (content, article_url): (Option<String>, Option<String>) = match article_type {
        ArticleSelector::All => (Some(row.get(9)?), Some(row.get(2)?)),
        ArticleSelector::Short => (Some(row.get(9)?), None),
        ArticleSelector::Article => (None, Some(row.get(2)?)),
    };
    Ok(Article {
        id: row.get(0)?,
        title: row.get(1)?,
